...
function AssessmentWorkspace({ assessment: initial, teachers, onBack, onSave }: { assessment: Assessment; teachers: Teacher[]; onBack: () => void; onSave: (assessment: Assessment, message?: string) => void }) {
  ...
  return <div className="page-wrap workspace-page">
    ...
    {stage === 'pra-observasi' && <RubricStage title="Telaah RPP / Modul Ajar" intro="Tinjau kesiapan perencanaan pembelajaran sebelum observasi berlangsung." items={preObservationItems} responses={assessment.preObservation} onResponse={(id, patch) => updateResponse('preObservation', id, patch)} />}
    {stage === 'observasi' && <RubricStage title="Observasi Pembelajaran" intro="Catat bukti pembelajaran yang terlihat selama observasi di kelas." items={observationItems} responses={assessment.observation} onResponse={(id, patch) => updateResponse('observation', id, patch)} evidenceLabel="Bukti pembelajaran" />}
